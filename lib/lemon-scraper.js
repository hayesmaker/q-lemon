...
module.exports = {
    getGameIdFromSearchPage: function (resp, name) {
        ...
        for (let i = 0; i < $gamesListHtml.length; i++) {
            games.push({
                gameTitle: $gamesListHtml[i].children[0].data.toLowerCase(),
        ...
        if (games.length === 1) {
            ...
            let matchedGame = _.find(games, {gameTitle: name.toLowerCase()});
            if (matchedGame) {
                return matchedGame
            }
        }
    ...
    getGameInfoFromGamePage: function(resp) {
        ...
        let $metaDataTable = $mainTable.find("tbody td").eq(1).find("table table").eq(1).find("table");//.find("table tbody tr td").html();
